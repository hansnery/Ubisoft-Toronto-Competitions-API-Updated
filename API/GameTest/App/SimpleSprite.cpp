...
CSimpleSprite::CSimpleSprite(const char *fileName, unsigned int nColumns, unsigned int nRows)
    : m_nColumns(nColumns), m_nRows(nRows), m_flipX(false), m_flipY(false)
...
void CSimpleSprite::SetFlip(bool flipX, bool flipY)
{
    m_flipX = flipX;
    m_flipY = flipY;
}
...
{            
    ...
    // Apply flip by adjusting scale
    float flipScaleX = m_flipX ? -1.0f : 1.0f;
    float flipScaleY = m_flipY ? -1.0f : 1.0f;
    
    glPushMatrix();
    glTranslatef(x, y, 0.0f);   
    glScalef(scalex * flipScaleX, scaley * flipScaleY, 1.0f);
